...
class RecursiveFibonacci {
  ...
  std::unique_ptr<RecursiveFibonacciAlgorithm>
      algorithm_;  // pointer to algorithm functor
 ...
 public:
  ...
  RecursiveFibonacci(long long int n, Algorithm algorithm = Algorithm::CXX)
    ...
    switch (algorithm) {
      ...
      case Algorithm::SERIAL:
        ...
        break;
      default:
        throw std::invalid_argument("Specified algorithm does not exist!");
        break;
    }
  }
  ...
  {
    algorithm_ = std::make_unique<RecursiveFibonacciAlgorithm>(new
  OpenMPAlgorithm(omp_num_threads));
  }
  */
